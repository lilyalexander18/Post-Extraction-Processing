...
* 
* Date created: 30 March 2018
* Date updated: 19 April 2018
*
...
***********************
	...
	cd "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/PrEP/"
	
* 1: Load in both data files (change title of excel file as appropriate)
***********************

* Costs data sheet
	import excel extraction_templates/GHCC_Extraction_PrEP_v05_20-Apr-18.xlsx, firstrow sh("Cost data") cellrange(A4) case(l)
...
* Study Attributes data sheet	
	import excel extraction_templates/GHCC_Extraction_PrEP_v05_20-Apr-18.xlsx, firstrow sh("Study attributes") cellrange(A4) case(l)
...
* First change directory to pull up generic inflation do file
	...
	*GDP Deflator
	 run do_files/inflation_methods/inflation_prep_gdp.do
...
******************************************
			...
			* First must simplify some of the var names for easier coding
			...
			replace ar_narrow="pat_mixed" if ar_narrow == "(patient) mixed"
			replace ar_narrow="pat_prod_loss_mixed" if ar_narrow=="productivity loss (mixed)"
...
*!*					*drop _
				*clean up
				foreach i of varlist cap_start_up-tot_full_costing_total {
					replace `i'=. if `i'==0
				}
...
use temp_dta/costs.dta
			...
			* Now fix variable names to get rid of symbols
			...
			replace si_narrow="med_equip" if si_narrow=="equipment (medical/intervention)"
...
use temp_dta/costs.dta
		...
		* Narrow activity cost categories
			...
			* Now fix variable names to get rid of symbols
			...
			replace a_narrow="adhreten" if a_narrow=="adherence/retention"
			...
			replace a_narrow="prep_tx_delivery" if a_narrow == "for prep: treatment delivery"
			
			tostring ancillary-primary_sd, replace
	
			foreach i of varlist ancillary-primary_sd {
					replace `i'="" if `i'=="0" | `i'=="."
	...
	* Now create binaries for the reshape command (need to finalize)
			*********************************************
				...
				*clean up
				foreach i of varlist anc_unspecified-prisd_prep_tx_delivery{
					replace `i'=. if `i'==0
				}
			...
			*Now replace all cat and subcat binaries with values of mean_cost
				...
				*STILL NEED TO REORDER VARS
				...
				foreach i of varlist ancillary-primary_sd {
					move `i' anc_unspecified
				}
				
				
			* Apply all mean_costs to these variables with missing obs for all 	
				foreach i of varlist ancillary-prisd_prep_tx_delivery{
					replace `i'=mean_cost if `i'!=.
				}
			*
...
****************************************
		...
		use temp_dta/study_attributes.dta, clear
		...
		* DROP NR for following Variables for encoding:
			...
			* Generate a new country string variable so you can merge in other information later:
			...
			foreach i of varlist extractor_initials article_dataset study_type econ_perspective_report econ_perspective_actual research_costs unrelated_costs overhead econ_costing real_world country geo_sampling_incountry country_sampling site_sampling px_sampling sample_size_derived timing exclusions personnel_dt iso_code currency_x traded volunteer_time family_time px_time aggregation subgroup scale scale_up seasonality sensitivity_analysis limitations coi ownership pop_sex id_class id_type id_int id_modality method px_costs_measured cat_cost costing_purpose_cat asd_costs pop_density int_description time_unit consistency controls cd4_med tb_rx_resistance id_phase id_tech{
			*replace `i' = lower(`i')
			replace `i'="." if `i'=="NR"
			encode `i', gen(`i'_1) label(`i')
			move `i'_1 `i'
			...
			* Additional recent variables (categorical)
			foreach i of varlist pop_ses pop_education tb_prev cd4_range {
				replace `i'="." if `i'=="NR" | `i'=="N/A"
			}
...
* And destring remaining numeric variables:
			*Years
				...
				replace year_intro=lower(year_intro)
				replace year_intro="" if year_intro=="n/a" | year_intro=="nr" |  year_intro=="na" | year_intro=="no year"
				destring year_intro, replace
...
*************************************************************************
	...
	*************************************************************************
	...
	use temp_dta/costs.dta
		...
		drop ar_capital-a_prisd_prep_tx_delivery
	merge m:1 id using temp_dta/study_attributes.dta
		...
		************************************************************************************
			...
			*Need to collapse one var at a time and append to a new dataset
				...
				save temp_dta/costs_temp.dta,replace
					...
					use temp_dta/costs.dta
				*set trace on
				foreach i of varlist ar_capital-a_prisd_prep_tx_delivery {
			...
			********************************************
			...
			save "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/PrEP/wide_files/prep_wide_file_Apr2018.dta", replace
