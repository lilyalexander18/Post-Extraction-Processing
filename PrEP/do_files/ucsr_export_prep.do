...
set more off 
	...
	*Lily's temporary path: 
	cd "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/PrEP/wide_files" 
	use prep_wide_file_Apr2018.dta, replace
...
**********************************
	...
	**********************************************************
	...
	egen check = rowtotal(a_primary_sd a_operational a_ancillary)
	gen diff = check - mean_cost
	gen flag = 1 if diff > 1  | diff < -1
...
** Begin by selecting key variables for export
	...
	*/
		...
		* Label remaining confusing variables
		...
		label var a_prisd_prep_tx_delivery "Primary SD: PrEP - Treatment Delivery (A)"
		label var a_prisd_for_prep__hct "Primary SD: PrEP - HCT (A)"
		...
		******************************
		...
		gen a_anc_adhereten = . 
		label var a_anc_adhereten "Ancillary: Adherence/Retention (A)" 
...
local to_decode "disease output_unit2 pop_density ownership platform facility_cat id_class id_phase id_type id_int method id_tech pop_sex tb_rx_resistance timing country_sampling geo_sampling_incountry site_sampling px_sampling sample_size_derived controls econ_perspective_actual econ_costing real_world asd_costs sensitivity_analysis scale research_costs unrelated_costs overhead volunteer_time family_time px_costs_measured cat_cost iso_code currency_iso currency_x inflation" 
...
****************************************************************************
	...
	replace referrals_flag = 1 if referrals != "." & referrals != "No" & !(strmatch(id_activities, "*eferrals*"))
...
a_primary_sd  a_prisd_prep_tx_delivery a_prisd_for_prep__hct a_secondary_sd a_secsd_unspecified a_ancillary /// 
a_anc_demand_generation a_anc_adhereten a_anc_unspecified /// 
...
* Finally, export!
**************************
save prep_clean_wide_file_Apr2018.dta, replace
