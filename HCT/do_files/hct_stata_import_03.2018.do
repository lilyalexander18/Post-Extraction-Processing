...
* Study Attributes data sheet	
	import excel using extraction_templates/GHCC_Extraction_HCT_MERGED_v08_20-Apr-18.xlsx, firstrow sh("Study attributes") cellrange(A4) case(l)
		...
		*Drop blank rows 
		drop if id == ""
		replace currency_yr = "" if currency_yr == "NR"
		destring currency_yr, replace
		*Save for working later
...
*!*					*drop _
				*clean up
				foreach i of varlist cap_building-tot_full_costing_total {
					replace `i'=. if `i'==0
				}
...
use temp_dta/costs.dta
			...
			*********************************************
				...
				*clean up
				foreach i of varlist cap_building_space-uns_unspecified {
					replace `i'=. if `i'==0
				}
...
use temp_dta/costs.dta
		...
		* Narrow activity cost categories
			...
			* Now fix variable names to get rid of symbols
			...
			replace a_narrow="unspec_counseling" if a_narrow=="for htc: counseling (counseling unspecified)"
			replace a_narrow="hiv_rapid_test" if a_narrow=="for htc: hiv rapid testing" 
			replace a_narrow="htc_service_delivery" if a_narrow=="for htc: htc service delivery"
			replace a_narrow="post_test_counseling" if a_narrow=="for htc: post-test counseling"
			replace a_narrow="demand_generation" if a_narrow=="demand generation"
	...
	* Now create binaries for the reshape command (need to finalize)
			*********************************************
				...
				*clean up
				foreach i of varlist anc_bldg_equip-uns_unspecified {
					replace `i'=. if `i'==0
				}
		...
		* id_facility (coding structure, from codebook)
			...
			* And binaries for faclity_category
			tab facility_cat, gen(v_)
				...
				rename v_11 mobile_clinic 
				rename v_12 client_residence
...
*************************************************************************
	...
	* First set up the desired order of the collapsed cost variable with markers
			...
			* Narrow Activity costs
				gen narrow_activity=.
				move narrow_activity a_anc_bldg_equip
