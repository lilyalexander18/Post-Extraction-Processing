...
***********************
	...
	cd "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/Retention_and_adherence"
* 1: Load in both data files (change title of excel file as appropriate)
***********************

* Costs data sheet
	import excel extraction_templates/GHCC_Extraction_Retention&Adherence_v7_19-Apr-2018.xlsx, firstrow sh("Cost data") cellrange(A4) case(l)
...
* Study Attributes data sheet	
	import excel extraction_templates/GHCC_Extraction_Retention&Adherence_v7_19-Apr-2018.xlsx, firstrow sh("Study attributes") cellrange(A4) case(l)
...
* First change directory to pull up generic inflation do file
	...
	*GDP Deflator
	 run do_files/inflation_methods/inflation_reten_adh_gdp.do
...
*!*					*drop _
			...
			*Now replace all cat and subcat binaries with values of mean_cost
				...
				rename recservices_technology_maintenan recserv_tech_maint
...
use temp_dta/costs.dta
			...
			*********************************************
				...
				*clean up
				foreach i of varlist  cap_equip_non_medical_inter-rec_other {
					replace `i'=. if `i'==0
				}
...
use temp_dta/costs.dta
		...
		* Narrow activity cost categories
			...
			* Now fix variable names to get rid of symbols
			...
			replace a_narrow="rc_counseling" if a_narrow=="for r&a: counseling"

			tostring combo-primary_sd, replace
	
			foreach i of varlist combo-primary_sd {
					replace `i'="" if `i'=="0" | `i'=="."
	...
	* Now create binaries for the reshape command (need to finalize)
			*********************************************
				...
				*clean up
				foreach i of varlist com_combo-prisd_rc_counseling {
					replace `i'=. if `i'==0
				}
			...
			* Apply all mean_costs to these variables with missing obs for all 	
				foreach i of varlist combo-prisd_rc_counseling{
					replace `i'=mean_cost if `i'!=.
				}
...
*******************************
		...
		* Numeric variables to recode NR to missing and destring to numeric
			foreach i of varlist lower_ci upper_ci std_dev median_cost lower_iqr upper_iqr input_price input_quantity program_level_cost time_period_mo{
			replace `i' = lower(`i')
...
*************************************************************************
	...
	*************************************************************************
	...
	clear
	use temp_dta/costs.dta
		drop ar_capital-a_prisd_rc_counseling
	merge m:1 id using temp_dta/study_attributes.dta
		...
		************************************************************************************
			...
			*Need to collapse one var at a time and append to a new dataset
				...
				save temp_dta/costs_temp.dta,replace
					...
					use temp_dta/costs.dta
				*set trace on
				foreach i of varlist ar_capital-a_prisd_rc_counseling {
			...
			********************************************
			...
			save  "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/Retention_and_adherence/wide_files/retention_wide_file_Apr2018.dta", replace
