...
***********************
	...
	cd "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/Post_violence_care"
	
* 1: Load in both data files (change title of excel file as appropriate)
***********************

* Costs data sheet
	import excel using extraction_templates/GHCC_Data_Extraction_Post-violence_care_v6_18-Apr-2018.xlsx, firstrow sh("Cost data") cellrange(A4) case(l)
...
* Study Attributes data sheet	
	import excel using extraction_templates/GHCC_Data_Extraction_Post-violence_care_v6_18-Apr-2018.xlsx, firstrow sh("Study attributes") cellrange(A4) case(l)
...
* First change directory to pull up generic inflation do file
	...
	*GDP Deflator
	 run do_files/inflation_methods/inflation_post_vio_care_gdp.do
...
use temp_dta/costs.dta
			...
			*********************************************
				...
				*clean up
				foreach i of varlist cap_building_space-rec_other {
					replace `i'=. if `i'==0
				}
			...
			*Now replace all cat and subcat binaries with values of mean_cost
				...
				rename cap_equipment__non_medical_inter  cap_nonmed_equip
			* Apply all mean_costs to these variables with missing obs for all 	
...
use temp_dta/costs.dta
		...
		* Narrow activity cost categories
			...
			* Now fix variable names to get rid of symbols
			...
			replace a_narrow="pv_care_hct" if a_narrow=="for pv care: hct" 
			replace a_narrow="pv_care_sup_package" if a_narrow == "for pv care: support packages" 
			replace a_narrow="pv_care_tx_delivery" if a_narrow == "for pv care: treatment delivery" 
	...
	* Now create binaries for the reshape command (need to finalize)
			*********************************************
				...
				*clean up
				foreach i of varlist anc_lab_services-secsd_unspecified {
					replace `i'=. if `i'==0
				}
			...
			*Now replace all cat and subcat binaries with values of mean_cost
				...
				*STILL NEED TO REORDER VARS
				...
				foreach i of varlist ancillary-secondary_sd {
					move `i' anc_lab_services
				}
				
				
			* Apply all mean_costs to these variables with missing obs for all 	
				foreach i of varlist ancillary-secsd_unspecified{
					replace `i'=mean_cost if `i'!=.
				}
			*
...
*************************************************************************
	...
	*************************************************************************
	...
	use temp_dta/costs.dta
		...
		drop ar_capital-a_secsd_unspecified
	merge m:1 id using temp_dta/study_attributes.dta
	...
	* First set up the desired order of the collapsed cost variable with markers
			...
			* Narrow Standardized Input costs
				gen narrow_stdinput=.
				move narrow_stdinput si_cap_building_space
		...
		************************************************************************************
			...
			*Need to collapse one var at a time and append to a new dataset
				...
				save temp_dta/costs_temp.dta,replace
					...
					use temp_dta/costs.dta
				*set trace on
				foreach i of varlist ar_capital-a_secsd_unspecified {
			...
			********************************************
			...
			save "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/Post_violence_care/wide_files/violence_wide_file_Apr2018.dta", replace
