...
** drew's import:
import excel extraction_templates/GHCC_Data_Extraction_Compiled_ART_Pt1_v04_18-Apr-2018.xlsx, firstrow sh("Cost data") cellrange(A4) case(l)
...
* Study Attributes data sheet	
	import excel extraction_templates/GHCC_Data_Extraction_Compiled_ART_Pt1_v04_18-Apr-2018.xlsx, firstrow sh("Study attributes") cellrange(A4) case(l) clear
...
* First change directory to pull up generic inflation do file
	...
	*GDP Deflator
	 run do_files/inflation_methods/inflation_art_gdp.do
...
*******************************
		...
		* Encode reporting standards variables for long version
			*set trace on
			...
			foreach i of varlist cost_source_rs cost_allocation_method_rs resource_id_rs price_sources_rs inputq_source_rs full_subsidized_rs adjustment_method_rs data_collection_rs recall_period_rs output_methods_rs data_timing_rs inflation_rs inflation_method_rs amortization_rs currency_rs pot_distortions_rs{
			replace `i' = lower(`i')
	...
	* First set up the desired order of the collapsed cost variable with markers
		...
		************************************************************************************
			...
			* Save version before importing GDPPC data:
			save wide_files/ART_wide_file_Apr2018.dta, replace
