...
* Costs data sheet
	import excel using extraction_templates/GHCC_STI_Management_v7_MERGED_19-Apr-18.xlsx, firstrow sh("Cost data") cellrange(A4) case(l)
...
* Study Attributes data sheet	
	import excel using extraction_templates/GHCC_STI_Management_v7_MERGED_19-Apr-18.xlsx, firstrow sh("Study attributes") cellrange(A4) case(l)
...
* First change directory to pull up generic inflation do file
	...
	*GDP Deflator
	 run do_files/inflation_methods/inflation_sti_gdp.do
...
*!*					*drop _
				*clean up
				foreach i of varlist cap_building-veh_maint_and_util {
					replace `i'=. if `i'==0
				}
			...
			* Apply all mean_costs to these variables with missing obs for all 	
				foreach i of varlist capital-veh_maint_and_util{
...
use temp_dta/costs.dta
		...
		*Activity Broad Categories
			...
			*First, make the broad names (that will be in use from HIV) usable
			...
			replace a_broad="primary_sd" if a_broad=="primary service delivery, unspecified" 
			replace a_broad="operational" if a_broad=="operational, unspecified"
		...
		* Narrow activity cost categories
			...
			* Now fix variable names to get rid of symbols
			...
			replace a_narrow="sti_syndrome_tx" if a_narrow=="for sti: sti syndrome treatment"
			
			tostring combo-unspecified, replace
				
		
			foreach i of varlist combo-unspecified {
					replace `i'="" if `i'=="0" | `i'=="."
	...
	* Now create binaries for the reshape command (need to finalize)
			*********************************************
				...
				*clean up
				foreach i of varlist com_combo-uns_unspecified {
					replace `i'=. if `i'==0
				}
			...
			* Apply all mean_costs to these variables with missing obs for all 	
				foreach i of varlist combo-uns_unspecified{
					replace `i'=mean_cost if `i'!=.
				}
		...
		* id_facility (coding structure, from codebook)
			...
			* And binaries for faclity_category
			tab facility_cat, gen(v_)
				...
				rename v_2 hosp_clinic
				rename v_3 hosp_clinic_disease
		...
		************************************************************************************
			...
			*2. Now collapse all of the categorical variables just by total
				...
				* (This could be done in a better way, problem is the above procedure reverses the order)	
				...
				narrow_activity a_narrow a_com_combo a_mix_mixed a_ope_bldg_equip a_ope_mass_education a_ope_unspec a_ope_supervision a_ope_training a_ope_transportation a_ope_unspecified  a_prisd_sti_syndrome_tx a_uns_unspecified
			...
			********************************************
			...
			save  "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/STI/wide_files/STI_wide_file_Apr2018.dta", replace
