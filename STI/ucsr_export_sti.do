...
label define region 1 "Latin America" 2 "Africa" 3 "Asia"
...
********************
	...
	*Need to replace all missing values in categoricals with 999 so we can label them "."
	foreach i of varlist ownership platform id_class id_type id_modality disease id_tech_diag id_tech_treat id_tech_prevention id_phase int_services country region pop_density pop_sex costing_purpose_cat timing country_sampling geo_sampling_incountry site_sampling px_sampling sample_size_derived controls econ_perspective_report econ_perspective_actual econ_costing real_world asd_costs research_costs unrelated_costs overhead volunteer_time family_time iso_code currency_x {
			replace `i'=999 if `i'==.
			label define `i' 999 ".", add
			}
	
	*Need to do same with string variables
	foreach i of varlist id unit_cost lead_author ref_author title journal_etc url id_facility int_description_long location ss_unique_trait id_pop_dem_std pop_age pop_ses pop_education hiv_prev cd4_range tb_prev list_asd_costs overhead_costs uncertainty_rmk {
		replace `i'="." if `i'==""
		}
...
**************************
export excel study-a_secsd_hct using final_dta/UCSR_export_full.xlsx, first(varl) missing(".") replace       
		...
		STOOP		
			...
			*Have to re-drop the values that needed to be dropped to do the aggregation to avoid confusion:
				...
				*And save this for editing into final collapsed dataset
				...
				export excel study-a_secsd_hct using final_dta/UCSR_export_collapsed.xlsx, first(varl) missing(".") replace   
