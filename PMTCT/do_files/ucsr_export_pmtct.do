...
**************************************************************************
**************************************************************************
** Do file for export of PMTCT studies to the UCSR, using select variables
** Lily Alexander
...
set more off 
	...
	*Lily's temporary path: 
	cd "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/PMTCT/wide_files" 
	use PMTCT_wide_file_Apr2018.dta, clear 
...
**********************************
	...
	****************************************************************
	egen check = rowtotal(si_capital si_personnel si_mixed si_recurrent si_unspecified)
	replace check = si_combined if check == 0
	gen diff = check - mean_cost
	gen flag = 1 if diff > 1.0
	...
	************************************************************************
	...
	local prefix "cap com mix per rec uns " 
	...
	**********************************************************
	...
	egen check = rowtotal(a_ancillary a_operational a_primary_sd a_secondary_sd a_unspecified a_mixed)
	gen diff = check - mean_cost
	gen flag = 1 if diff > 1.0
	...
	******************************************************************
	...
	local prefix "anc ope prisd secsd uns mix" 
...
** Begin by selecting key variables for export
	...
	*/
		...
		* Label remaining confusing variables
		...
		label var si_uns_med_int_supplies "Unspecified: Medical supplies (excluding drugs) (SI)"
		...
		label var a_secsd_antenatal_care "Secondary SD: Antenatal Care (A)" 
		label var a_secsd_neonate_infant_care "Secondary SD: Neonatal Infant Care (A)"
		label var a_secsd_for_pmtct__hct "Secondary SD: For PMTCT - HCT (A)"
		...
		//label var a_mix_mixed "Mixed: Mixed (A)"
		
		label var a_unspecified "Unspecified (A)"
		label var a_uns_unspecified "Unspecified: Unspecified (A)"
...
local to_decode "disease output_unit2 pop_density ownership platform facility_cat id_class id_phase id_int id_type method id_tech pop_sex tb_rx_resistance timing country_sampling geo_sampling_incountry site_sampling px_sampling sample_size_derived controls econ_perspective_actual econ_costing real_world asd_costs sensitivity_analysis scale research_costs unrelated_costs overhead volunteer_time family_time px_costs_measured cat_cost iso_code currency_iso currency_x inflation" 
...
foreach var of varlist location clinical_monitoring demand_generation counseling_content staff_type supportive_care visits referrals id_activities list_asd_cost overhead_costs method id_activities { 
...
si_capital si_cap_medical_equip si_cap_nonmed_equip si_cap_build si_cap_vehic si_cap_other si_mixed si_unspecified si_uns_med_int_supplies /// 
a_primary_sd a_prisd_lab_monitoring a_prisd_arv_delivery a_secondary_sd a_secsd_antenatal_care a_secsd_neonate_infant_care a_secsd_for_pmtct__hct a_secsd_unspecified a_ancillary /// 
...
* Finally, export!
**************************
save PMTCT_clean_wide_file_Apr2018.dta, replace
...
omitted_costs 
 Omitted Costs, Intervention Details, and Costed Activities
