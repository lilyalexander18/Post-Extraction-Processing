...
* Costs data sheet
	import excel using extraction_templates/GHCC_Data_Extraction_PMTCT_v11_18-Apr-2018.xlsx, firstrow sh("Cost data") cellrange(A4) case(l)
...
* Study Attributes data sheet	
	import excel using extraction_templates/GHCC_Data_Extraction_PMTCT_v11_18-Apr-2018.xlsx, firstrow sh("Study attributes") cellrange(A4) case(l)
...
* First change directory to pull up generic inflation do file
	...
	*GDP Deflator
	 run do_files/inflation_methods/inflation_pmtct_gdp.do
...
*!*					*drop _
			...
			*Now replace all cat and subcat binaries with values of mean_cost
				...
				order id-currency_rs capital-total cap_admin_equip-tot_full_costing_total
...
use temp_dta/costs.dta
	...
	* Now create binaries for the reshape command (need to finalize)
			...
			*Now replace all cat and subcat binaries with values of mean_cost
				...
				order id-currency_rs ar_capital-si_uns_med_int_supplies ancillary-unspecified anc_unspecified-uns_unspecified
			* Apply all mean_costs to these variables with missing obs for all 	
	...
	* First set up the desired order of the collapsed cost variable with markers
		...
		************************************************************************************
			...
			* Save version before importing GDPPC data:
			save final_dtas/PMTCT_wide_file_Apr2018.dta, replace
