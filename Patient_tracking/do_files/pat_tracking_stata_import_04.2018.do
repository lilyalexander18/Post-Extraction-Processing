...
***********************
	clear all
	cd "/Users/lilyalexander/Dropbox/ALL LIFE THINGS/INSP/Work with Sergio/GHCC/Post-Extraction-Processing/Patient_tracking"
	
* 1: Load in both data files (change title of excel file as appropriate)
***********************

* Costs data sheet
	import excel extraction_templates/GHCC_Extraction_Patient_Tracking_v7_18-Apr-2018.xlsx, firstrow sh("Cost data") cellrange(A4) case(l)
	drop if id == ""
...
* Study Attributes data sheet	
	import excel extraction_templates/GHCC_Extraction_Patient_Tracking_v7_18-Apr-2018.xlsx, firstrow sh("Study attributes") cellrange(A4) case(l)
	drop if id == ""
		*Save for working later
...
* First change directory to pull up generic inflation do file
	...
	*GDP Deflator
	 run do_files/inflation_methods/inflation_pat_tracking_gdp.do
...
use temp_dta/costs.dta
			...
			* Now fix variable names to get rid of symbols
			...
			tostring combined-recurrent, replace
				...
				foreach i of varlist combined-recurrent {
					replace `i'="" if `i'=="0" | `i'=="."
			...
			*********************************************
				...
				*clean up
				foreach i of varlist com_unit_cost_total-rec_other {
					replace `i'=. if `i'==0
				}
			...
			*Now replace all cat and subcat binaries with values of mean_cost
				foreach i of varlist combined-recurrent {
					...
					drop combined-recurrent
					rename unique* *
					
					*STILL NEED TO REORDER VARS
				foreach i of varlist combined-recurrent {
					move `i' com_unit_cost_total
				}
				
			* Apply all mean_costs to these variables with missing obs for all 	
				foreach i of varlist combined-rec_other{
					replace `i'=mean_cost if `i'!=.
				}
			*
...
use temp_dta/costs.dta
		...
		* Narrow activity cost categories
			...
			* Now fix variable names to get rid of symbols
			...
			replace a_narrow="pt_tracking_services" if a_narrow=="for pt tracking: tracking services"
	...
	* Now create binaries for the reshape command (need to finalize)
			*********************************************
				...
				*clean up
				foreach i of varlist com_combo-prisd_pt_tracking_services {
					replace `i'=. if `i'==0
				}
			...
			* Apply all mean_costs to these variables with missing obs for all 	
				foreach i of varlist combo-prisd_pt_tracking_services {
					replace `i'=mean_cost if `i'!=.
				}
			*
...
*******************************
		...
		* Numeric variables to recode NR to missing and destring to numeric
			foreach i of varlist lower_ci upper_ci std_dev median_cost lower_iqr upper_iqr input_price input_quantity program_level_cost {
			replace `i' = lower(`i')
...
*************************************************************************
	...
	*************************************************************************
	...
	clear
	use temp_dta/costs.dta
		drop ar_capital-a_prisd_pt_tracking_services
	merge m:1 id using temp_dta/study_attributes.dta
	...
	* First set up the desired order of the collapsed cost variable with markers
			...
			* Narrow Standardized Input costs
				gen narrow_stdinput=.
				move narrow_stdinput si_com_unit_cost
		...
		************************************************************************************
			...
			*Need to collapse one var at a time and append to a new dataset
				...
				save temp_dta/costs_temp.dta,replace
					...
					use temp_dta/costs.dta
				*set trace on
				foreach i of varlist ar_capital-a_prisd_pt_tracking_services {
