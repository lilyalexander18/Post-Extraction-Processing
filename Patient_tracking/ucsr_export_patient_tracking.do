...
**************************************************************************
**************************************************************************
** Do file for export of Patient Tracking studies to the UCSR, using select variables
** Lily Alexander
** Univ of Washington 
** January 2018 
		...
		*label var year_intro "Year Introduced at Study Site"
		...
		label var disease "Disease"
			...
			* Fix disease and capitalization
			...
			gen disease_new = 1 if disease == "HIV"
			label define disease1 1 "HIV"
			label values disease_new disease1
			drop disease
			rename disease_new disease
...
replace region = 1 if country_new == "South Africa"

drop country_new

label variable region "Region"
label define region 1 "SSA"
...
********************
	...
	*Need to replace all missing values in categoricals with 999 so we can label them "."
	foreach i of varlist ownership platform id_class id_type id_tech* id_modality disease id_phase int_services country region pop_density pop_sex timing country_sampling geo_sampling_incountry site_sampling px_sampling sample_size_derived controls econ_perspective_report econ_perspective_actual econ_costing real_world research_costs unrelated_costs overhead volunteer_time family_time iso_code currency_x tb_rx sensitivity_analysis {
	...
	*Need to do same with string variables
	...
	foreach i of varlist id unit_cost lead_author ref_author title journal_etc url ownership id_facility platform id_class id_modality int_description_long location ss_unique_trait id_pop_dem_std pop_age pop_ses pop_education hiv_prev cd4_range tb_prev list_asd_costs overhead_costs uncertainty_rmk tb_prev{
		...
		* Order variables thusly:
		order mean_cost si_capital si_combined si_personnel si_recurrent si_cap_medical_equip si_cap_nonmed_equip si_cap_other si_com_unit_cost_total si_per_mixed_unspec si_per_service_delivery si_per_support si_rec_building_space si_rec_med_int_supplies si_rec_nonmed_int_supplies a_ancillary a_mixed a_operational a_primary_sd a_secondary_sd a_anc_demand_generation a_anc_lab_services a_anc_unspecified a_mix_mixed a_ope_bldg_equip a_ope_logistics a_ope_program_mgmt a_ope_supervision a_ope_training a_ope_transportation a_ope_unspecified a_prisd_circumcision_proced a_prisd_unspecified a_secsd_hct
		...
		* Label remaining confusing variables
		...
		label var si_combined "Mixed (SI)"
		...
		// check with this one: 
		label var si_com_unit_cost_total "Mixed: Mixed (SI)"
...
**************************
export excel study-a_secsd_hct using patient_tracking_clean_wide_file.xlsx, first(varl) missing(".") replace       
